...
document
    ...
    .addEventListener("click", async function (e) {
        ...
        if (attempts >= 4) {
            ...
            let limit_container = document.querySelector(".max_limit")
            limit_container.style.display = 'block';
            let result_container = document.querySelector(".resultData")

            result_container.style.filter = "blur(5px)"
            return;
        }
        ...
        const url = `https://freetoolsv2.marktivities.guru/api/phone/validator/?country=${encodeURIComponent(
            countryISO
        ...
        let interval = setInterval(() => {
            if (currentProgress <= 100) {
                ...
                if (currentProgress === 100) {
                    // document.getElementById("validate").innerText = "Result for";
...
const maxLimit = document.querySelector('.max_limit');

function toggleMaxLimit() {
    maxLimit.style.display = finalResult.classList.contains('limitReached') ? 'block' : 'none';
}
